...
export default function BurgerIcon({ isOpen, variant = 'light', className }: BurgerIconProps) {
  ...
  return (
    <div className={clsx('flex h-5 w-6 flex-col items-center justify-center gap-1', className)}>
      ...
      <span
        className={clsx(
          'h-0.5 w-full transform transition-[transform] duration-300 ease-in-out motion-reduce:transition-none',
          lineColor,
      ...
      <span
        className={clsx(
          'h-0.5 w-full transition-[opacity] duration-300 ease-in-out motion-reduce:transition-none',
          lineColor,
          isOpen && 'opacity-0',
        )}
      />
